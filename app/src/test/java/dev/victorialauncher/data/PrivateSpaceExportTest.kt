 ...
 * not the space is locked at the moment someone exports. Every shape [Prefs] stores a private
 * key in gets a case here; [PrivateSpaceTest] covers the underlying "does this key belong to
 * that profile" decision this reuses.
 */
class PrivateSpaceExportTest {
    ...
    fun `a private favorite is removed and its neighbours survive in order`() {
        ...
        assertEquals(
            ...
            stripPrivateSpaceFromExport("favorites_order", raw, privateSerial),
        )
    }
    ...
    fun `favorites with no private key come back identical`() {
        ...
        assertSame(raw, stripPrivateSpaceFromExport("favorites_order", raw, privateSerial))
    }
    ...
    fun `a private hidden app is removed, the rest of the set survives`() {
        ...
        assertEquals(setOf(a, c), stripPrivateSpaceFromExport("hidden_apps", raw, privateSerial))
    }

    @Test
    fun `a hidden set with no private key comes back identical`() {
        val raw = setOf(a, c)
        assertSame(raw, stripPrivateSpaceFromExport("hidden_apps", raw, privateSerial))
    }
    ...
    fun `a rename for a private app is removed, other renames survive`() {
        ...
        assertEquals(
            """{"$a":"Alpha","$c":"Charlie"}""",
            stripPrivateSpaceFromExport("name_overrides_json", raw, privateSerial),
        )
    }
    ...
    fun `an icon override for a private app is removed, other overrides survive`() {
        ...
        assertEquals(
            ...
            stripPrivateSpaceFromExport("icon_overrides_json", raw, privateSerial),
        )
    }
    ...
    fun `a launch count for a private app is removed, other counts survive`() {
        ...
        assertEquals(
            """{"$a":"4","$c":"1"}""",
            stripPrivateSpaceFromExport("launch_counts_json", raw, privateSerial),
        )
    }
    ...
    fun `a map with no private key comes back identical`() {
        ...
        assertSame(raw, stripPrivateSpaceFromExport("name_overrides_json", raw, privateSerial))
    }
    ...
    fun `a private app inside a folder is removed, its folder-mates survive in order`() {
        ...
        val stripped = stripPrivateSpaceFromExport("folders_json", raw, privateSerial) as String
    ...
    fun `folders with no private member come back identical`() {
        ...
        assertSame(raw, stripPrivateSpaceFromExport("folders_json", raw, privateSerial))
    }
    ...
    fun `a quick-launch slot holding a private key is left out entirely`() {
        assertNull(stripPrivateSpaceFromExport("quick_launch_left_key", bPrivate, privateSerial))
        assertNull(stripPrivateSpaceFromExport("quick_launch_right_key", bPrivate, privateSerial))
    }

    @Test
    fun `a quick-launch slot holding an ordinary key survives`() {
        assertEquals(a, stripPrivateSpaceFromExport("quick_launch_left_key", a, privateSerial))
    }

    // --- cross-cutting behavior ---------------------------------------------------------------

    @Test
    fun `no private space to strip, serial null or zero, leaves every shape untouched`() {
        ...
        for (serial in listOf(null, 0L)) {
            assertSame(favorites, stripPrivateSpaceFromExport("favorites_order", favorites, serial))
            assertSame(hidden, stripPrivateSpaceFromExport("hidden_apps", hidden, serial))
            assertSame(renames, stripPrivateSpaceFromExport("name_overrides_json", renames, serial))
            assertSame(folders, stripPrivateSpaceFromExport("folders_json", folders, serial))
            assertEquals(bPrivate, stripPrivateSpaceFromExport("quick_launch_left_key", bPrivate, serial))
        }
    }

    @Test
    fun `a key from a different second profile is not the private space's and survives`() {
        // A work profile, serial 10, while the private space is serial 11: neither owns the
        // other's apps, so a work-profile key must not be caught by the private-space strip.
    ...
    fun `malformed JSON is passed through rather than crashing the export`() {
        ...
        assertSame(brokenFolders, stripPrivateSpaceFromExport("folders_json", brokenFolders, privateSerial))
        assertSame(brokenMap, stripPrivateSpaceFromExport("name_overrides_json", brokenMap, privateSerial))
    }

    @Test
    fun `an unrelated preference passes through untouched`() {
        assertEquals(56, stripPrivateSpaceFromExport("icon_size_dp", 56, privateSerial))
        assertEquals(true, stripPrivateSpaceFromExport("haptics_enabled", true, privateSerial))
    }
    ...
    fun `a pinned-shortcut key for the private profile is stripped`() {
        ...
        assertEquals(a, stripPrivateSpaceFromExport("favorites_order", raw, privateSerial))
    }
